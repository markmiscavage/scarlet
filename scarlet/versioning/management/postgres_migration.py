# -----------------------------------------
# Specific PostgreSQL commands for migrations
# -----------------------------------------
...
class PostgresBackend(object):
    ...
    def _trigger_function(self, base_model, version_model, args):
    ...
    def do_updates(self, m):
        if getattr(m._meta, '_is_view', None):
            ...
            trigger_sql = self._trigger_function(m._meta._base_model, m._meta._version_model, args)
            cursor.execute(trigger_sql)
            cursor.execute(self.TRIGGER % args)
