                ...
                if version_model:
                    if isinstance(value, models.ManyToManyField):
                        ...
                        if not isinstance(value, M2MFromVersion):
                            raise TypeError(
                                "The model %s cannot contain a ManyToManyField use M2MFromVersion instead"
                                % name
                            )
                    ...
                    if (
                        ...
                        value.remote_field.related_name = (
                            "%s_version" % value.remote_field.related_name
                        )
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for base in bases:
            if issubclass(base, models.Model) and not base._meta.abstract:
                raise TypeError(
                    "You cannot use a non abstract base model %r with a VersionView model %s."
                    % (base, name)
                )

            if issubclass(base, BaseModel):
                raise TypeError(
                    "VersionView model %r can't inherit from a BaseModel %r"
                    % (cls, base)
                )
            ...
            if issubclass(base, BaseVersionedModel):
                if not issubclass(type(base), VersionViewMeta):
                    if custom_version:
                        raise TypeError(
                            "Django inheritence will only allow VersionView model %r to inherit from one and only one BaseVersionedModel"
                            % cls
                        )
                    else:
        ...
        version_model = SharedMeta(
            "%s_version" % name, tuple(version_bases), versioned_attrs
        )
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if not getattr(meta, "abstract", False):
            if not getattr(meta, "_base_model", None) or not issubclass(
                ...
                raise TypeError(
                    "%s must declare a subclass of BaseModel as \
...
class BaseVersionedModel(Cloneable, Schedulable):
    ...
    def schedule(self, when=None, action=None, **kwargs):
        ...
        """
        action = "_publish"
        super(BaseVersionedModel, self).schedule(when=when, action=action, **kwargs)
