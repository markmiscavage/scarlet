   ...
   */
	renderOption: function(isLoading) {
		return {
			...
			option: (item, escape) => {
				return (
					'<div data-id=' + item['id'] + '>' + escape(item['text']) + '</div>'
				)
			},
   ...
   */
	addItem: function(value, $item) {
		if (this.isMultiple) {
			...
			if ($item.attr('data-id') !== this.singleInput.val())
   ...
   */
	createAddButton: function() {
		...
		const add = $('<a>')
			...
			.html(
				'<i class="fa fa-plus-circle" aria-hidden="true"></i>Add ' + this.name
			)
